...
void main(int argc, char **argv) {
  ...
  int iv2_len; 
  unsigned char *iv2 = HexHandler(argv[2], &iv2_len);

  unsigned char *number = "\x00\x00\x00\x02";
  ...
  memcpy(key, material + 16, 16);
 

  // extract iv1 and construct counter

  memcpy(iv1, material + 36, iv1_len);
  
  memcpy(counter, iv1, iv1_len);
  memcpy(counter + 4, iv2, iv2_len);
  memcpy(counter + 12, number, 4);

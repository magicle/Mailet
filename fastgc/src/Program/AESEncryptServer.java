...
public class AESEncryptServer extends ProgServer {
  ...
  // Test by Shuai 
  OutputStream testoutstream = new FileOutputStream("/home/shuai/workspace/fastgc/server.txt");
  for (int i = 0; i<clps.length; i++) {
//    testoutstream.write((byte[])Character.toChars(i));
    testoutstream.write((byte)'\n');
    Utils.writeBigInteger(clps[i][0], bytelength, testoutstream);
    testoutstream.write((byte)'\n');
    Utils.writeBigInteger(clps[i][1], bytelength, testoutstream);
    testoutstream.write((byte)'\n');
  }
